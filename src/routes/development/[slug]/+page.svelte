...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
	...
	<!-- Build trigger — when spec is done and there's no live build:
	     either no build has started yet (!workspaceUuid), or the workspace was
	     lost (workspaceUuid set but its metadata is gone, e.g. wiped on restart).
	     The build endpoint clears the stale link and starts a fresh build. -->
	{#if idea.specStatus === 'completed' && (!idea.workspaceUuid || !wsMeta)}
				...
				<div class="flex items-center gap-3">
					...
					<div>
						{#if idea.workspaceUuid && !wsMeta}
							<p class="text-sm font-medium text-white">Build needs to be re-run</p>
							<p class="text-xs text-white/50">This app's previous build is no longer available. Start the autonomous AI build again — approved mockups are used as the design reference.</p>
						{:else}
